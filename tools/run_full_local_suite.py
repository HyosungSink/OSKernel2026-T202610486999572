    ...
    *,
    ...
    if quick_reuse_only:
        ...
        with FileLock(kernel_lock_path):
            require_logged_success(
                "build kernels",
                ["make", "kernel-rv", "kernel-la", "la-bios"],
                cwd=ROOT,
    ...
    for arch, runtime in sorted(variants_needed):
...
def refresh_official_rootfs_wrappers(
    ...
    allowed = {(group, runtime) for group, runtime, _ in map(parse_sample, arch_samples)}
        ...
        for group in GROUP_ORDER:
            ...
            if allowed:
                ...
                if group == "lmbench" and not lmbench_raw_script_has_bench_env(raw_path):
                    return True
                if group == "ltp":
                    if not ltp_raw_script_uses_runtest(runtime_root):
                        return True
            ...
            if script_path.exists() or raw_path.exists():
                return True
        if runtime == "glibc" and groups and glibc_runtime_support_is_stale(runtime_root):
            return True
...
def assert_official_rootfs_ready(
    ...
    if official_rootfs_is_stale(rootfs_dir, arch_samples, ltp_selected_cases, ltp_start_case):
...
def official_allowed_runtime_groups(arch_samples: list[str]) -> list[str]:
    return sorted({
        f"{runtime}:{group}"
        for group, runtime, _ in map(parse_sample, arch_samples)
    })
...
def build_official_rootfs_dir(
    ...
    allowed = {(group, runtime) for group, runtime, sample_arch in map(parse_sample, samples) if sample_arch == arch}
    if glibc_rootfs is not None:
        copy_tree_contents(glibc_rootfs, glibc_dir)
    else:
        glibc_dir.mkdir(parents=True, exist_ok=True)
    if musl_rootfs is not None:
        copy_tree_contents(musl_rootfs, musl_dir)
    else:
        musl_dir.mkdir(parents=True, exist_ok=True)
...
def run_official_arch(
    ...
    image_path = logs_dir / f"official-{arch}.qcow2"
    ...
    try:
        returncode, timed_out, stop_reason = run_qemu(
            official_qemu_command(arch, image_path, image_format="qcow2"),
            raw_log_path,
            effective_timeout,
            ...
            if parallel_ltp_shard_mode:
                ...
                for arch in ARCH_ORDER:
                    ...
                    official_rootfs_root = (
                        SHARED_CACHE_ROOT if args.quick_point_retest else WORK_ROOT
                    ) / "rootfs-official"
                    official_rootfs_root.mkdir(parents=True, exist_ok=True)
                    ...
                    needs_rebuild = (
                        args.rebuild_rootfs
                        or not marker.exists()
                        or official_rootfs_is_stale(out_dir, arch_samples, ltp_selected_cases, ltp_start_case)
                    )
                    if needs_rebuild:
                        ...
                        assert_official_rootfs_ready(out_dir, arch_samples, ltp_selected_cases, ltp_start_case)
                        marker.write_text(datetime.now(timezone.utc).isoformat(), encoding="utf-8")
                        ...
                        assert_official_rootfs_ready(out_dir, arch_samples, ltp_selected_cases, ltp_start_case)
                        complete_stage(f"reuse official rootfs {arch}")
                ...
                for arch in ARCH_ORDER:
                    ...
                    while pending_arch_samples:
                        ...
                        needs_rebuild = (
                            args.rebuild_rootfs
                            or not marker.exists()
                            or (bool(ltp_selected_cases) and not args.quick_point_retest)
                            or (bool(ltp_start_case) and not args.quick_point_retest)
