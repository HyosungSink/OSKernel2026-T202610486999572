...
ALLOWED_HIDDEN_PAYLOAD_PREFIXES = (
    "kernel/starry-next/vendor/",
    "kernel/starry-next/patches/",
    "kernel/starry-next/embedded-runtime-refresh/",
)
...
def is_hidden_path(rel_path: str) -> bool:
    for prefix in ALLOWED_HIDDEN_PAYLOAD_PREFIXES:
        base = prefix[:-1] if prefix.endswith("/") else prefix
        if rel_path == base or rel_path.startswith(prefix):
            return False
...
def filtered_tracked_payload_hashes(src_root: Path, excludes: list[str]) -> tuple[dict[str, dict[str, object]], dict[str, object]]:
    ...
    for rel_path in tracked:
        ...
        if is_excluded_path(rel_path, excludes):
            ...
            continue
        if src.is_symlink():
            payload[rel_path] = {"kind": "symlink", "target": os.readlink(src)}
        else:
            mode = src.stat().st_mode
            payload[rel_path] = {
                "kind": "file",
    ...
    return dict(sorted(payload.items())), manifest
...
def command_hash_submission(args: argparse.Namespace) -> int:
    src_root = repo_root(args.source_root)
    excludes = DEFAULT_EXCLUDES + args.exclude
    payload, manifest = filtered_tracked_payload_hashes(src_root, excludes)
...
def command_expand_subset(args: argparse.Namespace) -> int:
    ...
    if not sample_names:
        ...
        arch_order = ARCH_ORDER if args.arch == "both" else [args.arch]
        sample_names = [
            Sample(group=group, runtime=runtime, arch=arch).name
            for group in GROUP_ORDER[start_index:]
...
def command_audit_submodules(args: argparse.Namespace) -> int:
    ...
    for entry in entries:
        ...
        if ok and args.check_remote:
            ...
            remote_ok = False
            for attempt in range(1, args.remote_retries + 1):
                try:
                    proc = subprocess.run(
                    ...
                    attempts.append(
                        {
                            "attempt": attempt,
                            "timeout": args.remote_timeout,
                            "stderr": f"timed out after {args.remote_timeout}s",
                        }
                    )
                if attempt < args.remote_retries:
                    time.sleep(args.remote_retry_delay)
            result["remote_publicly_readable"] = remote_ok
            if not remote_ok:
                if any("timeout" in item for item in attempts):
                ...
                result["remote_attempts"] = len(attempts)
        if not ok:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    hash_submission = subparsers.add_parser(
        "hash-submission",
        help="Hash the filtered submission payload directly from a repository checkout.",
    )
    hash_submission.add_argument("--out", help="Optional JSON output path.")
    hash_submission.add_argument("--manifest-out", help="Optional manifest JSON output path.")
    hash_submission.add_argument(
        "--source-root",
        help="Source repository root. Defaults to the repository containing this script.",
    )
    hash_submission.add_argument(
        ...
        action="append",
        default=[],
        help="Additional repo-relative directory to exclude from the submission payload.",
    )
    hash_submission.set_defaults(func=command_hash_submission)
    ...
    expand.add_argument(
        "--start-group",
        choices=GROUP_ORDER,
        help="When no target sample is given, only expand groups starting from this group.",
    )
    expand.add_argument(
        "--arch",
        choices=["both", *ARCH_ORDER],
        default="both",
        help="When no target sample is given, limit expanded samples to this architecture.",
    )
    ...
    audit.add_argument(
        ...
        type=int,
        default=3,
        help="Retry count for unauthenticated git ls-remote checks.",
    )
    audit.add_argument(
        "--remote-retry-delay",
        type=float,
        default=2.0,
        help="Delay seconds between unauthenticated git ls-remote retries.",
    )
